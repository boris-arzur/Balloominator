...
// Use SPI to communicate with ADXL345
#define ADXL345_I2C 0

// Use I2C to communicate with ADXL345
//#define ADXL345_I2C 1

#define ADXL345_SPI_CS 8                // SPI chip select
#define ADXL345_SPI_READ_MODE (1<<7)    // SPI read command
#define ADXL345_SPI_MULTI_BYTE (1<<6)   // SPI multi-byte command

// these are to disable interrupts when reading/writing to SPI. this will 
// prevent collisions on SPI if multiple devices are present. if the exact spi devices are known
// then this can be modified to just turn off the interrupt of the other spi devices
#define ADXL345_ENTER_CRIT()    {unsigned char volatile saved_sreg = SREG; cli()
#define ADXL345_LEAVE_CRIT()    SREG = saved_sreg;}
