...
#define ADXL345_SPI_ENB()   do {digitalWrite(ADXL345_SPI_CS, LOW);} while (0)
#define ADXL345_SPI_DIS()   do {digitalWrite(ADXL345_SPI_CS, HIGH);} while (0)
...
void ADXL345::powerOn() {
  ...
  #else
    // setup the SPI to Mode 3
    // setup SPI clock 
    SPI.begin();
    SPI.setDataMode(SPI_MODE3);
    SPI.setClockDivider(SPI_CLOCK_DIV16);

    pinMode(ADXL345_SPI_CS, OUTPUT);
    digitalWrite(ADXL345_SPI_CS, HIGH);
  #endif
...
void ADXL345::readFrom(byte address, int num, byte _buff[]) 
{
    unsigned char i;
    address |= ADXL345_SPI_READ_MODE;
    address |= (num > 1) ? ADXL345_SPI_MULTI_BYTE : 0;
